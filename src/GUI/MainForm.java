...
public class MainForm extends JFrame implements MouseListener {
    ...
    public MainForm() {
        ...
        getRootPane().setBorder(BorderFactory.createMatteBorder(2, 2, 2, 2, new Color(72, 21, 167, 255)));

        ImageIcon logo = new ImageIcon(getClass().getResource("/images/icon/main_logo.png"));
        setIconImage(logo.getImage());

        // ======================== Menu =======================
        String[] navItemInfo = {
            "seperate", "2", "", "",
            "Bán điện thoại", "icon/buy.png", "qlBanHang", "qlBanHang",
            "Nhập điện thoại", "icon/import_products.png", "qlNhapHang", "qlNhapHang",
            "seperate", "2", "", "",
            "Điện thoại", "icon/smartphones.png", "xemSanPham", "qlSanPham",
            "Loại điện thoại", "icon/classify_smartphones.png", "xemLoaiSanPham", "qlLoaiSanPham",
            "Hóa đơn", "icon/bills.png","xemPheuNhap", "qlHoaDon",
            "Phiếu nhập", "icon/import_coupon.png", "xemPheuNhap", "qlPhieuNhap",
            "Khuyến mãi", "icon/promotions.png", "xemKhuyenMai", "qlKhuyenMai",
            "seperate", "1", "", "",
            "Nhân viên", "icon/employees.png", "xemNhanVien", "qlNhanVien",
            "Khách hàng", "icon/customers.png", "xemKhachHang", "qlKhachHang",
            "Nhà cung cấp", "icon/nha_san_xuat.png", "xemNCC", "qlNCC",
            "seperate", "1", "", "",
            "Tài khoản", "icon/accounts.png", "xemTaiKhoan", "qlTaiKhoan",
            "Quyền", "icon/permission.png", "xemQuyen", "qlQuyen",
            "seperate", "1", "", "",
            "Thống kê", "icon/statisticals.png", "", "",
        ...
        int headerBg = 30;
        int headerH = 55;
        Color color=new Color(86, 45, 134);
        Color colorHover=new Color(109, 63, 161);
        header = new NavBarContainer(new Rectangle(0, 0, WIDTH, headerH));
        header.setBackground(color);

        headerTitle = new NavBarTitle(new Rectangle((WIDTH - 400) / 2, 0, 400, headerH), "CỬA HÀNG SMARTPHONE");
        headerTitle.setColorDefault(new Color(200, 200, 200));
        headerTitle.setBgDefault(color);
        ...
        int btnWidth = 50;
        int iconSize = 30;
        NavBarButton btnClose = new NavBarButton(new Rectangle(WIDTH - btnWidth, 0, btnWidth, headerH), "", "icon/header_x.png");
        btnClose.setIconLocation(new Rectangle((btnWidth - iconSize) / 2, (headerH - iconSize) / 2, iconSize, iconSize));
        btnClose.setBgDefault(color);
        ...
        // Minimize Button
        NavBarButton btnMinimize = new NavBarButton(new Rectangle(WIDTH - btnWidth * 2, 0, btnWidth, headerH), "", "icon/minimize.png");
        btnMinimize.setIconLocation(new Rectangle((btnWidth - iconSize) / 2, (headerH - iconSize) / 2, iconSize, iconSize));
        btnMinimize.setBgDefault(color);
        btnMinimize.setBgHover(colorHover);
        ...
        if (LoginForm.taiKhoanLogin != null) {
            ...
            NavBarButton btnLogout = new NavBarButton(new Rectangle(0, 0, menuW - btnWidth, headerH), tenNhanVien, "icon/logout.png");
            btnLogout.setBgDefault(color);
            btnLogout.setBgHover(colorHover);
            btnLogout.relocate2();
            ...
            NavBarButton btnSettingUser = new NavBarButton(new Rectangle(menuW - btnWidth, 0, btnWidth, headerH), "", "icon/setting.png");
            btnSettingUser.setIconLocation(new Rectangle((btnWidth - iconSize) / 2, (headerH - iconSize) / 2, iconSize, iconSize));
            btnSettingUser.setBgDefault(color);
            btnSettingUser.setBgHover(colorHover);
